...
The repository contains two complementary organizations of the same project:

- `pipelines/` preserves stage-oriented source code in the order supplied by
  the authors;
- `figures/` contains code that can be defensibly associated with a figure or
  panel.

The stage-oriented workflows are upstream analyses. They should not be cited as
exact figure-reproduction code unless they also appear in `FIGURE_CODE_MAP.tsv`.
...
Absolute paths and scheduler commands are retained for provenance. They must be
